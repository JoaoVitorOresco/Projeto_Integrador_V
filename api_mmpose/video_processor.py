...
# --- Bloco de importação para o YouTube Uploader ---
import sys
import traceback
...
try:
    from projeto_integrador_V.youtube_uploader import get_authenticated_service, upload_video as upload_video_to_youtube_api
    youtube_uploader_available = True
    print("INFO (video_processor.py): Módulo 'projeto_integrador_V.youtube_uploader' carregado com sucesso.")
except ImportError as e_yt_import:
    youtube_uploader_available = False
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    print(f"ERRO DETALHADO AO IMPORTAR 'projeto_integrador_V.youtube_uploader':")
    print(f"Tipo do Erro: {type(e_yt_import)}"); print(f"Mensagem do Erro: {e_yt_import}")
    print("Traceback Completo:"); traceback.print_exc()
    if hasattr(e_yt_import, 'name'): print(f"  Nome do módulo problemático: {e_yt_import.name}")
    if hasattr(e_yt_import, 'path'): print(f"  Caminho do módulo problemático: {e_yt_import.path}")
    print(f"sys.path atual: {sys.path}")
    print("!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!")
    def get_authenticated_service(request_django=None): return None
    def upload_video_to_youtube_api(service, file_path, title, description, tags, category_id, privacy_status): return None
# --- Fim do bloco de importação do YouTube Uploader ---
...
def process_video_with_mmpose(video_instance, django_request_object=None):
    if not ApiMmposeConfig.model_loaded or not ApiMmposeConfig.inferencer:
        video_instance.processing_status = 'failed'; video_instance.processing_log = 'Modelo MMPose não disponível.'; video_instance.save(); return

    print(f"--- INICIANDO TESTE 5: ESCRITA DE VÍDEO DESACOPLADA (Frames Originais) ---")
    ...
    # Nome do arquivo de vídeo para este teste
    output_video_filename = f"{original_filename_base}_processed_TEST5_{timestamp}.mp4"
    processed_video_upload_path_relative_to_media = os.path.join('videos', 'processed_tests', output_video_filename)
    output_video_full_path = os.path.join(settings.MEDIA_ROOT, processed_video_upload_path_relative_to_media)
    os.makedirs(os.path.dirname(output_video_full_path), exist_ok=True)

    video_instance.processing_status = 'processing_test5_decoupled'
    video_instance.processing_log = "Iniciando Teste 5: Processando dados e armazenando frames originais em memória."
    video_instance.save()

    cap = None; out = None; frame_count = 0 
    
    # Listas para armazenar dados e frames
    all_original_frames_bgr = [] 
    frame_data_objects_to_create_db = [] 
    
    processing_error_occurred = False

    try:
        # === LOOP 1: Ler frames, processar dados com MMPose, armazenar frames originais e dados ===
        print("INFO (T5): Iniciando Loop 1 - Leitura e Processamento de Dados MMPose...")
        ...
        while cap.isOpened():
            ...
            if not ret: break 
            frame_count += 1
            if frame_count % 100 == 0: print(f"INFO (T5): Processando dados do frame {frame_count}...")
            
            all_original_frames_bgr.append(frame_bgr_original) # Armazena o frame original BGR

            # Processa o frame com MMPose para obter dados (como no seu código original)
        ...
        if cap: cap.release() # Fecha o leitor de vídeo após o Loop 1
        print(f"INFO (T5): Loop 1 concluído. {frame_count} frames lidos e dados processados. {len(all_original_frames_bgr)} frames armazenados.")

        # Salva os dados de FrameData no banco de dados
        if frame_data_objects_to_create_db:
            ...
            try: FrameData.objects.bulk_create(frame_data_objects_to_create_db, batch_size=500); print(f"INFO (T5): {len(frame_data_objects_to_create_db)} FrameData salvos.")
            except Exception as e_db: print(f"ERRO (T5): bulk_create FrameData: {e_db}"); video_instance.processing_log+=f"\nErro DB (T5): {e_db}"
        else: print("INFO (T5): Nenhum FrameData para salvar.")

        # === LOOP 2: Escrever os frames BGR originais armazenados para um novo arquivo de vídeo ===
        if not all_original_frames_bgr:
            raise ValueError("Nenhum frame foi armazenado em memória para escrita.")

        print(f"INFO (T5): Iniciando Loop 2 - Escrita dos {len(all_original_frames_bgr)} frames originais armazenados para {output_video_full_path}...")
        fourcc = cv2.VideoWriter_fourcc(*'mp4v')
        out = cv2.VideoWriter(output_video_full_path, fourcc, fps_original, (width_original, height_original))
        if not out.isOpened():
            # Tenta fallback para AVI se mp4v falhar
            output_video_filename_avi = f"{original_filename_base}_processed_TEST5_{timestamp}.avi" 
            processed_video_upload_path_relative_to_media_avi = os.path.join('videos', 'processed_tests', output_video_filename_avi) 
            output_video_full_path_avi = os.path.join(settings.MEDIA_ROOT, processed_video_upload_path_relative_to_media_avi) 
            ...
            if not out.isOpened(): raise IOError(f"Não foi possível abrir VideoWriter para {output_video_full_path} ou {output_video_full_path_avi} no Loop 2")
                ...
                output_video_filename = output_video_filename_avi
                processed_video_upload_path_relative_to_media = processed_video_upload_path_relative_to_media_avi
                output_video_full_path = output_video_full_path_avi
        
        for original_frame in all_original_frames_bgr:
            out.write(original_frame)
        
        if out: out.release() # Fecha o escritor de vídeo após o Loop 2
        print(f"INFO (T5): Loop 2 concluído. Vídeo com frames originais salvo em: {output_video_full_path}")
        video_instance.processed_video_file.name = processed_video_upload_path_relative_to_media

    except Exception as e:
        print(f"ERRO EXCEPCIONAL (Teste 5): {e}"); traceback.print_exc()
        video_instance.processing_status = 'test5_failed_processing_loop'; video_instance.processing_log += f"\nExceção (T5 ProcLoop): {e}"
        processing_error_occurred = True
    finally: # Garante que cap e out sejam liberados se foram abertos no try principal
        if cap and cap.isOpened(): cap.release()
        if out and out.isOpened(): out.release() # Se out foi aberto no try e não fechado no Loop 2

    if not processing_error_occurred:
        video_instance.processing_status = 'test5_completed_ready_for_yt'
        video_instance.processing_log += f'\nProcessamento de dados e escrita de vídeo (T5) concluídos. {frame_count} frames.'
        
        # Lógica de Upload para YouTube (mantida)
        if youtube_uploader_available and os.path.exists(output_video_full_path):
            print(f"INFO (T5): Tentando upload {output_video_full_path} para YouTube...")
            youtube_service = get_authenticated_service(request_django=django_request_object)
            if youtube_service:
                video_title_yt = f"TESTE5 DecoupledWrite {video_instance.title or original_filename_base}"
                video_description_yt = f"Teste 5 (Escrita Desacoplada, Frames Originais) - {video_instance.title or original_filename_base}"
                tags_yt = ["teste5", "esgrimetrics_debug", "decoupled"]
                video_instance.youtube_upload_status = 'pending_yt_test5'; video_instance.save() 
                youtube_id_str = upload_video_to_youtube_api( 
                    service=youtube_service, file_path=output_video_full_path, title=video_title_yt,
                    description=video_description_yt, tags=tags_yt, category_id="28", privacy_status="unlisted" 
                )
                if youtube_id_str:
                    video_instance.youtube_video_id = youtube_id_str; video_instance.youtube_upload_status = 'uploaded_yt_test5'
                    video_instance.processing_log += f"\nYT Teste5 ID: {youtube_id_str}"; print(f"INFO (T5): YT Upload ID: {youtube_id_str}")
                else: 
                    video_instance.youtube_upload_status = 'failed_yt_test5'; video_instance.processing_log += "\nFalha YT (T5)."
                    print("ERRO (T5): Falha YT upload.")
            else: 
                video_instance.youtube_upload_status = 'auth_req_yt_test5'; video_instance.processing_log += "\nAuth YT (T5)."
                print("AVISO (T5): Auth YT necessária.")
        elif not youtube_uploader_available: video_instance.youtube_upload_status = 'module_missing_yt_test5'
        elif not os.path.exists(output_video_full_path): video_instance.youtube_upload_status = 'no_file_yt_test5'
    
    video_instance.save() 
    print(f"INFO (T5): Estado final salvo. ProcStatus: {video_instance.processing_status}, YTStatus: {video_instance.youtube_upload_status}")
    print(f"--- FIM DO TESTE 5 ---")
